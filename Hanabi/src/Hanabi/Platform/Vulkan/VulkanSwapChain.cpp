...
namespace Hanabi
{
	...
	void VulkanSwapChain::InitSurface(GLFWwindow* windowHandle)
	{
		...
		HNB_CORE_ASSERT(graphicsQueueNodeIndex != UINT32_MAX);
		HNB_CORE_ASSERT(presentQueueNodeIndex != UINT32_MAX);
	...
	void VulkanSwapChain::Create(uint32_t width, uint32_t height, bool vsync)
	{
		...
		HNB_CORE_ASSERT(presentModeCount > 0);
